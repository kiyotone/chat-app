...
function ContactList() {
  ...
  return (
    <div className="w-[16rem] items-center">
      <div className="flex flex-col pt-6 items-center">
        <div className="relative flex items-center">
          <input
            type="text"
            className="bg-[#16171b] pl-9 px-0 placeholder:text-gray-500 p-4 rounded-[1rem]"
            placeholder="Search"
          />
      ...
      <div className="flex flex-col ml-4 mt-7">
        <div className="flex items-center justify-between text-[15px] text-gray-500">
          ...
          <div
            className="hover:bg-[#16171b] rounded-full mr-3"
            onClick={toggleUnread}
          >
            {Unread ? (
              <MdOutlineKeyboardArrowUp className="insert-y-0 right-0" />
            ) : (
              <MdOutlineKeyboardArrowDown className="insert-y-0 right-0" />
            )}
          </div>
        </div>
        {Unread && (
